...
`geodesic`/`barycenter`/`analysis` API dispatching over `method="socp"`
and `method="sinkhorn"`. Shooting (Phase 5) is next, and is intended to
become the default method: it is often substantially faster than the SOCP,
which stays as the exact fallback for densities that are not strictly
positive. Until it lands `method=` is required, so that the default
changes once rather than twice. This is a deliberate divergence from the
Julia package, which defaults to `:socp`. Each step lands as its own reviewed, tested PR —
